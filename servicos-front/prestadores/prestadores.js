...
async function buscaDaApiExibeNaTabela() {
    const res = await fetch(`${URL_BASE_API}/prestadores/listar`);
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        btEx.innerText = 'Apagar';
        btEx.setAttribute('data-id', data[i].id);
        btEx.addEventListener('click', solicitaExclusaoDeServicoNaApi);
        td1.innerText = data[i].nick;
        td2.innerText = data[i].email;
        td3.innerText = data[i].servico;
...
async function preencheDadosParaEdicao(e) {
    ...
    const res = await fetch(`${URL_BASE_API}/prestadores/um`, opt);
    const data = await res.json();
    console.log(data);
    document.forms[0].id.value = data.id;
    document.forms[0].nick.value = data.nick;
    document.forms[0].email.value = data.email;
    document.forms[0].servico.value = data.servico;
}
...
async function enviaDadosParaCadastroOuEdicaoNaApi(e) {
    ...
    const res = await fetch(`${URL_BASE_API}/prestadores/${op}`, opt);
    const data = await res.json();
    e.target.reset();
...
async function solicitaExclusaoDeServicoNaApi(e) {
    const dados = new FormData();
    dados.append('id', e.target.getAttribute('data-id'));
    const opt = {
        method: 'post',
    ...
    const res = await fetch(`${URL_BASE_API}/prestadores/excluir`, opt);
    const data = await res.json();
    alert(data.status);
    buscaDaApiExibeNaTabela();
}
