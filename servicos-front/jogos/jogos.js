...
async function buscaDaApiExibeNaTabela() {
    const res = await fetch(`${URL_BASE_API}/jogos/listar`);
...
async function preencheDadosParaEdicao(e) {
    ...
    const res = await fetch(`${URL_BASE_API}/jogos/um`, opt);
    const data = await res.json();
    console.log(data);
...
async function enviaDadosParaCadastroOuEdicaoNaApi(e) {
    ...
    const res = await fetch(`${URL_BASE_API}/jogos/${op}`, opt);
    const data = await res.json();
    e.target.reset();
...
async function solicitaExclusaoDeServicoNaApi(e) {
    ...
    const res = await fetch(`${URL_BASE_API}/jogos/excluir`, opt);
    const data = await res.json();
    alert(data.status);
    buscaDaApiExibeNaTabela();
}
